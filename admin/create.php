...
try {
    ...
    $errorMessage = 'データベース接続エラー: ' . htmlspecialchars($e->getMessage());
    $allCategories = [];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['thumbnail']) && $_FILES['thumbnail']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['thumbnail']['tmp_name'], $targetPath)) {
            ...
            $errorMessage = "サムネイルのアップロードに失敗しました";
        }
    } else {
    ...
    if (empty($errorMessage)) {
        try {
            ...
            $pdo->rollBack();
            $errorMessage = '保存時にエラーが発生しました: ' . htmlspecialchars($e->getMessage());
        }
    }
}
?>
...
<?php include './common/sidenav.php'; ?>
  ...
  <?php endif; ?>
    ...
    <form id="blogForm" method="POST" action="" enctype="multipart/form-data">
      ...
      <input type="text" id="title" style="width: 100%;" name="title" required value="<?= htmlspecialchars($title) ?>">
      <label for="content">本文</label>
      <div id="content"></div>
      <input type="hidden" id="hiddenContent" name="content" value="<?= htmlspecialchars($content) ?>">

      <label for="newCategory">カテゴリ</label>
        <input type="text" id="newCategory" name="newCategory" value="<?= htmlspecialchars($_POST['newCategory'] ?? '') ?>">
  ...
  <?php endforeach; ?>
        ...
        <div id="thumbnailError" style="color: red; margin-top: 5px;"></div>
...
quillContent.root.innerHTML = document.getElementById('hiddenContent').value;
